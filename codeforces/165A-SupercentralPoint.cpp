...
void solve() {
	...
	vpi v(n);
	mivi x_store, y_store;
	for(int i = 0;i < n;i++) {
		cin >> v[i].ff >> v[i].ss;
		x_store[v[i].ff].pb(v[i].ss);
		y_store[v[i].ss].pb(v[i].ff);
	}
	...
	for(int i = 0;i < n;i++) {
		...
		for(auto it : x_store[v[i].ff]) {
			if(it < v[i].ss) f1 = true;
			if(it > v[i].ss) f2 = true;
		}

		for(auto it : y_store[v[i].ss]) {
			if(it < v[i].ff) f3 = true;
			if(it > v[i].ff) f4 = true;
		}
