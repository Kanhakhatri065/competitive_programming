...
void solve() {
    ...
    mem(freq, 0);
    mem(max_common_gcd, 0);

    int ans = 0;
    F0R(i, n) {
        for(int j = 1;j * j <= v[i];j++) {
            if(v[i] % j == 0) {
                freq[i] = max({freq[i], max_common_gcd[j] + 1, max_common_gcd[v[i] / j] + 1});
            }
        }

        for(int j = 1;j * j <= v[i];j++) {
            if(v[i] % j == 0) {
                max_common_gcd[j] = max(max_common_gcd[j], freq[i]);
                max_common_gcd[v[i] / j] = max(max_common_gcd[v[i] / j], freq[i]);
            }
        }
