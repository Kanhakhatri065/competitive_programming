...
void solve() {
    ...
    sort(all(store_neg), greater<int>());
    for(int i = sz(store_neg) - 1;i >= 0;i--) {
        if(k == 0) {
            break;
        }

        store_pos.pb(-store_neg.back());
        store_neg.pop_back();
        k--;
    }
    ...
    int sum = accumulate(all(store_pos), 0) + accumulate(all(store_neg), 0);
    pf(sum);
}
