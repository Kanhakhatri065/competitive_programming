...
vector <vector<int> > tr(401);
vector <vector<int> > cr(401);
 
bool tr_slots[1001][1001];
bool cr_slots[1001][1001];
bool used[1001];
void solve() {
    int n, m;
    cin >> n >> m;
    ...
    if ((tr_slots[1][n] || cr_slots[1][n]) && d[n] < 1e5) {
        cout << d[n] << endl;
    } else {
        cout << -1 << endl;
    }
