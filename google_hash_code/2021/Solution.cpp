...
int main() {
    ...
    for(int i = 0;i < I;i++) max_time_for_every_intersection[i] = INF;
    for(int i = 0;i <= D;i++) {
        if(int(time_fram_intersections[i].size()) > 0) {
            for(auto it : time_fram_intersections[i]) {
                if(it.second >= 1 && it.second <= D) {
                    max_time_for_every_intersection[it.first] = min(max_time_for_every_intersection[it.first], it.second);
                }
            }
        }
    }
    ...
    for(int i = 0;i < I;i++) {
        ...
        for(int v : end_points_of_roads[i]) {
            cout << street_name_from_id[v] << " ";
            if(max_time_for_every_intersection[v] == INF) {
                cout << 1;
            } else {
                cout << max_time_for_every_intersection[v];
            }
            cout << endl;
        }  
    }
